...
const Itinerary = () => {
  ...
  */
  ...
  {loggedIn && transportData.map((transport, idx) => (
    <div key={idx} className="item-box" onClick={() => setSelectedTransport(transport)}>
      <p><strong>Service: </strong></p>
      <p>Departure Date: {transport.departure_date}</p>
      <p>Return Date: {transport.return_date}</p>
      <p>Origin: {transport.from_place}</p>
  ...
  useEffect(() => {
    if (selectedTransport && mapInstance) {
      // Clear previous features
      mapInstance.getLayers().getArray()
        .filter(layer => layer instanceof VectorLayer)
        .forEach(layer => mapInstance.removeLayer(layer));
  
      // Create a new feature for the selected transport path
      const feature = new Feature({
        geometry: new Point(fromLonLat([-87.6298, 41.8781])) // Start of path
      });
  
      const pathCoordinates = [
        [-87.6298, 41.8781], // Example coordinate 1
        [-87.629, 41.879],   // Example coordinate 2
        // Add more hardcoded coordinates as needed
      ...
      // Adjust map view to focus on the path
      const view = mapInstance.getView();
      if (view) {
        view.fit(pathCoordinates, { padding: [50, 50, 50, 50], duration: 1000 });
      }
    }
  }, [selectedTransport, mapInstance]);
